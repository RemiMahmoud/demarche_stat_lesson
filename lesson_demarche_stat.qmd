...
author: "Rémi Mahmoud"
date: today
date-format: "[remi.mahmoud@institut-agro.fr] <br> [https://demarche-stat-lesson.netlify.app]"
format:
   revealjs:
    transition: slide
    theme: serif
...
L’association Air Breizh surveille la qualité de l’air et mesure la concentration de polluants comme l’ozone (O3, en $\mu g.m^{-3}$) ainsi que les conditions météorologiques comme la température, la nébulosité, le vent, etc. Durant l’été 2001, 112 données ont été relevées à Rennes.
...
```{r, class.source="my_class_code_big"}

ozone <- read.table("https://r-stat-sc-donnees.github.io/ozone.txt",header=TRUE, stringsAsFactors = TRUE)

# head(ozone)
print(head(ozone), right = FALSE, row.names = FALSE)
...
```{r, echo = TRUE}
#| classes: my_class_code_big
#| class-source: my_class_code_big
...
# donnees <- rnorm(500, mean = 0, sd = 2)

# # Histogramme + densité théorique
# ggplot(data.frame(x = donnees), aes(x)) +
#   geom_histogram(data = donnees, aes(y = after_stat(density)), bins = 30, fill = "skyblue", alpha = 0.5) +
#   stat_function(fun = dnorm, args = list(mean = 0, sd = 2), color = "black", size = 1.2) +
#   labs(title = "Distribution statistique vs loi normale",
#        y = "Densité",
#        x = "Valeurs") +
#   theme_bw()
...
plot_distrib_normal <- ggplot(df, aes(x=x, y=y)) +
  geom_line(size=1.2, color="black") +
  labs(title = "Loi normale",
       y = "Densité de probabilité",
       x = NULL) +
  theme_bw() +  
  # geom_histogram(data = data.frame(x = donnees), aes(y = after_stat(density)), bins = 30, fill = "skyblue", alpha = 0.5) +
  # stat_function(fun = dnorm, args = list(mean = 0, sd = 2), color = "black", size = 1.2) +
...
data_distrib <- tibble(distrib = rep(c("Normal", "Fisher", "Chi2", "Uniform"), each = 1000) ,
                       x =  c(seq(-2,12,length.out =1000), seq(0,4,length.out =1000), seq(0,15,length.out =1000), seq(1,5,length.out =1000)),
                       y = c(dnorm(seq(-2,12,length.out =1000), 5,2),
                             df(seq(0,4,length.out =1000),20,100),
                             dchisq(seq(0,15,length.out =1000),3),
                             dunif(seq(1,5,length.out =1000),1,5))) %>% 
  mutate(mode = x[which.max(y)], .by = distrib,
         mean = case_when(distrib == "Normal" ~ 5,
                          distrib == "Fisher" ~ (100/98),
                          distrib == "Chi2" ~ 3,
                          distrib == "Uniform" ~ 3
                          )) 
data_sample_distrib <- tibble(distrib = rep(c("Normal", "Fisher", "Chi2", "Uniform"), each = 10000) ,
                       x = c(rnorm(10000, 5,2),
                             rf(10000,20,100),
                             rchisq(10000,3),
                             runif(10000,1,5))) 
...
data_distrib %>% 
  ...
  geom_vline(aes(xintercept = mode), linetype = "dashed", color = "red") +
  labs(title = "Quelques exemples de lois de probabilité", subtitle = "En rouge le mode, en noir la moyenne", x = NULL, y= "Densité de probabilité") +
  theme(text = element_text(size = 12)) +
  geom_histogram(data = data_sample_distrib, aes(x = x, y =  after_stat(density)), alpha = .4)
```
...
-   **Règle 68-95-99** :
    -   68% des données se trouvent à ±1 écart-type ($\sigma$) de la moyenne.
    -   95% à ±2 écarts-types.
...
# Graphique de la distribution normale avec ggplot2
plot_distrib_normal +
  
  # Coloration des régions pour la règle 68-95-99
  geom_area(data=subset(df, x >= -2 & x <= 2), aes(y=y), fill="lightblue", alpha=0.5) +
  geom_area(data=subset(df, x >= -4 & x <= 4), aes(y=y), fill="lightgreen", alpha=0.4) +
  geom_area(data=subset(df, x >= -6 & x <= 6), aes(y=y), fill="lightcoral", alpha=0.3) +
  
  # Lignes verticales pour les écarts-types
  geom_vline(xintercept = c(-2, 2), linetype="dashed", color="blue") +
  geom_vline(xintercept = c(-4, 4), linetype="dashed", color="green") +
  geom_vline(xintercept = c(-6, 6), linetype="dashed", color="red") +
  geom_vline(xintercept = c(0), linetype="dashed", color="grey60") +
  
  # Lignes horizontales
  geom_segment( x =  -2, xend = 2, y  = 0.125, yend = 0.125, color="blue",  arrow = arrow(length = unit(0.02, "npc"))) +
  geom_segment( x =  2, xend = -2, y  = 0.125, yend = 0.125, color="blue",  arrow = arrow(length = unit(0.02, "npc"))) +
  geom_segment( x =  -4, xend = 4, y  = 0.075, yend = 0.075, color="green",  arrow = arrow(length = unit(0.02, "npc"))) +
  geom_segment( x =  4, xend = -4, y  = 0.075, yend = 0.075, color="green",  arrow = arrow(length = unit(0.02, "npc"))) +
  geom_segment( x =  -6, xend = 6, y  = 0.025, yend = 0.025, color="red",  arrow = arrow(length = unit(0.02, "npc"))) +
  geom_segment( x =  6, xend = -6, y  = 0.025, yend = 0.025, color="red",  arrow = arrow(length = unit(0.02, "npc"))) + 
  
  geom_label(x = 0, y = 0.135, label = latex2exp::TeX("$\\pm \\sigma$")) + 
  geom_label(x = 0, y = 0.085, label = latex2exp::TeX("$\\pm 2 \\times \\sigma$")) + 
  geom_label(x = 0, y = 0.035, label = latex2exp::TeX("$\\pm 3 \\times  \\sigma$")) + 
  
  labs(title = "Loi normale et règle des 68-95-99",
       y = "Densité de probabilité",
       x = NULL) 
...
echantillon_student <-tibble(x= seq(-4,4, by = 0.05), y = dt(x = x,df = 111))
...
echantillon_student %>% 
  ...
  geom_label(label = "Zone de rejet",x = -2.5, y=  0.02, size=5) +
  geom_label(label = "Zone de rejet",x = 2.5, y=  0.02, size=5) +
  geom_label(label = "Zone de non-rejet",x = 0, y=  0.2, size=5) +
  labs(x = NULL, y = NULL)
...
library(purrr)
list_ddl_student <- tibble(n1 = c(2,5, 10,30,50))

gen_data_student <- function(n1){
  return(tibble(x= seq(-5,5,by = 0.025), val_student = dt(x, df = n1)))
}

plot_student <- list_ddl_student %>% 
  mutate(data_val_student = map(n1,  gen_data_student)) %>% 
  tidyr::unnest(data_val_student) %>% 
  mutate(ddl = paste0("df =", n1)) %>% 
  mutate(ddl = forcats::fct_reorder(ddl, n1)) %>% 
  ggplot(aes(x = x, y= val_student, color = ddl)) + 
  geom_line(linewidth = 1.5, alpha = .5)+
  theme_bw()+ 
  labs(x = NULL, y=  "Densité") +
  theme(text = element_text(size =15))


vec_colors_ddl_student <- c( "#F8766D", "#A3A500", "#00BF7D", "#00B0F6", 
"#E76BF3", "black")

plot_student_with_normal <- list_ddl_student %>% 
  mutate(data_val_student = map(n1,  gen_data_student)) %>% 
  tidyr::unnest(data_val_student) %>% 
  mutate(ddl = paste0("df =", n1)) %>% 
  bind_rows(tibble(x = seq(-5,5,by = 0.025), val_student = dnorm(x), ddl = "N(0,1)", n1 = Inf)) %>% 
  mutate(ddl = forcats::fct_reorder(ddl, n1)) %>% 
  ggplot(aes(x = x, y= val_student, color = ddl)) + 
  geom_line(linewidth = 1.5, alpha = .5)+
  theme_bw()+ 
  theme(text = element_text(size =15)) + 
  labs(x = NULL, y=  "Densité") +
  scale_color_manual(values = vec_colors_ddl_student)

ggsave("images/plot_student_with_normal.png", plot = plot_student_with_normal, width = 10, height = 7, dpi = "retina")
ggsave("images/plot_student.png", plot = plot_student, width = 10, height = 7, dpi = "retina")
```
...
-   **$H_0$ hypothèse nulle**, $H_0: \mu_1 = \mu_2$ vs **$H_1$ hypothèse alternative**, $H_1: \mu_1 \neq\mu_2$
...
print(head(ozone[, c("maxO3", "vent")]), row.names =FALSE)
...
#| class-source: my_class_code
# LinearModel prémâche le travaille
round(LinearModel(maxO3 ~ pluie,
            data= ozone)$Ttest, digits = 2)
...
<!-- ![](images/decomp_anova_hospital.png){width="50%" fig-align="center" height="250"} -->
...
data_hospital <- tibble(Hospital = rep(c("A", "B", "C"), each = 20),
                        position_Hospital = rep(c(1:3), each = 20),
                        Time = c(rnorm(20, mean = 1.2, sd = 0.3),
                                 rnorm(20, mean = 1.9, sd = 0.3),
                                 rnorm(20, mean = 1.5, sd = 0.3))) %>% 
  mutate(mean_by_group  = mean(Time), .by = Hospital)  %>% 
  mutate(pos_point_jitter = position_Hospital + rnorm(60, mean = 0, sd= .05)) 

data_hospital_mean = data_hospital %>% summarise(Time = mean(Time))
data_hospital_mean_by_group = data_hospital %>% summarise(Time = mean(Time),  .by = c(Hospital, position_Hospital)) %>% 
  mutate(pos_segment = 1:3)

plot_waiting_time_hospital <- data_hospital %>%
  ggplot(aes(x = pos_point_jitter, y = Time)) +
  geom_point(color = "grey50", alpha = .4) +
  # geom_segment(data = data_hospital_mean_by_group, linewidth =  3, color = "red", alpha = .7, aes(x = pos_segment - 0.15, xend = pos_segment + 0.15)) +
  theme_bw() +
  labs(y = "Waiting time (hours)", title = "Waiting time by hospital", x =  "Hospital")+
  scale_x_continuous(breaks = c(1,2,3), labels = c("A", "B", "C")) 

plot_bss <- data_hospital %>% 
  ggplot(aes(x = pos_point_jitter, y = Time)) +
  # geom_point(color = "grey50", position = position_jitter(width = .05, height = 0), size=  2, alpha = .4) + 
  geom_point(color = "grey50", alpha = .4) + 
  theme_bw() + 
  geom_segment(data = data_hospital_mean_by_group, linewidth =  3, color = "grey20", alpha = .7, aes(x = pos_segment - 0.15, xend = pos_segment + 0.15)) + 
  geom_hline(yintercept = mean(data_hospital$Time), linetype = "dashed", linewidth = 2, color = "red", alpha = .4) + 
  geom_segment(data = data_hospital_mean_by_group,
               aes(x = position_Hospital, yend = Time), y=  mean(data_hospital$Time), linewidth = 2, color = "red", alpha = .5) + 
  labs(y = "Waiting time (hours)", title = latex2exp::TeX("Variabilité inter-groupe  ($SC_F$)"), x =  "Hospital")+
  scale_x_continuous(breaks = c(1,2,3), labels = c("A", "B", "C")) 



plot_wss <- data_hospital %>% 
  ggplot(aes(x = pos_point_jitter, y = Time)) +
  geom_point(color = "grey50", size=  2, alpha = .4) + 
  theme_bw() + 
  geom_segment(data = data_hospital_mean_by_group, linewidth =  3, color = "red", alpha = .7, aes(x = pos_segment - 0.15, xend = pos_segment + 0.15)) + 
  geom_segment(data = data_hospital_mean_by_group,
               aes(x = position_Hospital, yend = Time), y=  mean(data_hospital$Time), linewidth = 2, color = "red", alpha = .5) +
  geom_segment(data = data_hospital,
               aes(x = pos_point_jitter, yend = Time, y=  mean_by_group), linewidth = 1, color = "red", alpha = .5) + 
  labs(y = "Waiting time (hours)", title = latex2exp::TeX("Variabilité intra-groupe ($SC_R$)"), x =  "Hospital") +
  scale_x_continuous(breaks = c(1,2,3), labels = c("A", "B", "C")) 


plot_ss <- data_hospital %>% 
  ggplot(aes(x = pos_point_jitter, y = Time)) +
  geom_point(color = "grey50", size=  2, alpha = .4) + 
  theme_bw() + 
  # geom_segment(data = data_hospital_mean_by_group,
  #              aes(x = position_Hospital, yend = Time), y=  mean(data_hospital$Time), linewidth = 2, color = "red", alpha = .5) +
  geom_segment(data = data_hospital,
               aes(x = pos_point_jitter, yend = Time), y=  mean(data_hospital$Time), linewidth = 1, color = "red", alpha = .3) + 
  geom_hline(yintercept = mean(data_hospital$Time), linetype = "dashed", linewidth = 2, color = "red", alpha = .4) + 
  labs(y = "Waiting time (hours)", title = latex2exp::TeX("Variabilité totale ($SC_T$)"), x =  "Hospital") +
  scale_x_continuous(breaks = c(1,2,3), labels = c("A", "B", "C")) 



ggsave(plot =plot_waiting_time_hospital, filename = "images/plot_waiting_time_hospital.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
ggsave(plot =plot_ss, filename = "images/plot_ss_anova.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
ggsave(plot =plot_wss, filename = "images/plot_wss_anova.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
ggsave(plot =plot_bss, filename = "images/plot_bss_anova.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
...
:::: {#first-column}
::: r-stack
![](images/plot_waiting_time_hospital.png){.fragment}

![](images/plot_ss_anova.png){.fragment}
:::
::::
...
![](images/plot_bss_anova.png){.fragment}
...
![](images/plot_wss_anova.png){.fragment}
:::::::::::
...
\begin{tabular}{cccc}
 &$\displaystyle{\underbrace{\sum_{i,j}(Y_{ij}-Y_{\bullet \bullet})^2}_{SC_T}}$&$= \displaystyle{\underbrace{\sum_{i}n_i(Y_{i\bullet}-Y_{\bullet \bullet})^2}_{SC_F}}$&$+ \displaystyle{\underbrace{\sum_{i,j}(Y_{ij}-Y_{i\bullet})^2}_{SC_R}}$\\
...
$$\mbox{On a : }\mathbb{E}\left(\frac{SC_{mod}}{I-1}\right)=\sigma^2+\frac{1}{I-1}\displaystyle{\sum_{i=1}^I n_i\alpha_i^2} \quad ;\quad    \mathbb{E}\left(\frac{SC_{R}}{n-I}\right)=\sigma^2$$
...
fisher_data %>% 
  ...
  geom_label(x = 4, y = 0.025, label = "2.07%", color = "red", size=8) +
  geom_label(x = 0.7, y = 0.25, label = "Zone de non-rejet", color = "blue", size=8) +
  labs(x = NULL, y = NULL) + 
...
![](images/causality_shoes_0.png){.fragment}

![](images/causality_shoes_1.png){.fragment}

![](images/causality_shoes_2.png){.fragment}

<!-- ![](images/causality_shoes_3.png){.fragment} -->
...
```{r, eval = FALSE}
x = rnorm(100)
# tibble(val_x = rep(x, 6), y = val_x*rep(seq(-1,1,length.out = 3), each = 100) +  rnorm(600), beta =  rep(seq(-1,1,length.out = 6), each = 100)) %>% 
#   mutate(cor= cor(val_x,y), .by = beta)


data_cor = tibble(x = x, y = 3*x, cor = cor(x,y)) %>%
  bind_rows(tibble(x = x, y = 3*x  +3*rnorm(100), cor = cor(x,y)))%>%
  bind_rows( tibble(x = x, y = 3*x  +8*rnorm(100), cor = cor(x,y))) %>%
  bind_rows( tibble(x = x, y = - 3*x  +4*rnorm(100), cor = cor(x,y)))%>%
  bind_rows( tibble(x = x, y = 8*rnorm(100), cor = cor(x,y))) %>% 
  bind_rows( tibble(x = x, y = - 3*x  , cor = cor(x,y))) %>% 
...
::::::: columns
::: {.column width="45%"}
$$SCER = \sum_{i=1}^n (y_i - \hat{y}_i)^2 = \sum_{i=1}^n (y_i - \hat{\beta_0} - \hat{\beta_1} x_i)^2$$
...
data_ozone <- ozone %>% 
  mutate(fitted = fitted(lm(maxO3 ~T9, data = ozone)))
 
data_ozone_mean = data_ozone %>% summarise(maxO3 = mean(maxO3))
# data_ozone_mean_by_group = data_ozone %>% summarise(maxO3 = mean(maxO3),  .by = c(ozone, position_ozone)) %>% 
#   mutate(pos_segment = 1:3)

plot_maxO3_ozone <- data_ozone %>%
  ggplot(aes(x = T9, y = maxO3)) +
  geom_point(color = "grey30", alpha = .4) +
  theme_bw() +
  labs(y = "MaxO3", title = "MaxO3 against temperature at 9 AM", x =  "Temperature (°C)")

plot_SCR <- data_ozone %>% 
  ggplot(aes(x = T9, y = maxO3)) +
  # geom_point(color = "grey50", position = position_jitter(width = .05, height = 0), size=  2, alpha = .4) + 
  geom_point(color = "grey30", alpha = .4) + 
  geom_line(aes(y = fitted), linetype= "solid", color = "black", linewidth = 2) + 
  theme_bw() +
  # geom_hline(yintercept = mean(data_ozone$maxO3), linetype = "dashed", linewidth = 2, color = "red", alpha = .4) + 
  geom_segment(data = data_ozone,
               aes(x = T9, yend = fitted, y=  maxO3), linewidth = 1, color = "red", alpha = .3) + 
  labs(y = "MaxO3", title = latex2exp::TeX("Variabilité résiduelle  ($SC_R$)"), x =  "Temperature at 9 PM (°C)") 



plot_SCM <- data_ozone %>% 
  ggplot(aes(x = T9, y = maxO3)) +
  geom_point(color = "grey80", size=  2, alpha = .4) + 
  geom_point(color = "grey20",  alpha = .4, aes(y= fitted), size =3) + 
  theme_bw() + 
  geom_segment(data = data_ozone,
               aes(x = T9, yend = fitted), y=  mean(data_ozone$maxO3), linewidth = 1, color = "red", alpha = .3) +
  geom_hline(yintercept = mean(data_ozone$maxO3), linetype = "dashed", linewidth = 2, color = "red", alpha = .4) + 
  labs(y = "MaxO3",  title = latex2exp::TeX("Variabilité expliquée par le modèle ($SC_M$)"), x =  "Temperature at 9 PM (°C)", subtitle = "Points noirs: valeurs ajustées par le modèle") +
  theme(title = element_text(size = 9))


plot_SCT <- data_ozone %>% 
  ggplot(aes(x = T9, y = maxO3)) +
  geom_point(color = "grey30", size=  2, alpha = .4) + 
  theme_bw() + 
  geom_segment(data = data_ozone,
               aes(x = T9, yend = maxO3), y=  mean(data_ozone$maxO3), linewidth = 1, color = "red", alpha = .2) + 
  geom_hline(yintercept = mean(data_ozone$maxO3), linetype = "dashed", linewidth = 2, color = "red", alpha = .4) + 
  labs(y = "MaxO3", title = latex2exp::TeX("Variabilité totale ($SC_T$)"), x =  "Temperature (°C)")  



ggsave(plot =plot_SCM, filename = "images/plot_SCM_ozone.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
ggsave(plot =plot_SCR, filename = "images/plot_SCR_ozone.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
ggsave(plot =plot_SCT, filename = "images/plot_SCT_ozone.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
ggsave(plot =plot_maxO3_ozone, filename = "images/plot_ozone.png", dpi = "retina", width = 9, height = 8, unit = 'cm')
...
::: r-stack
![](images/plot_ozone.png){.fragment}

![](images/plot_SCT_ozone.png){.fragment}
:::
...
![](images/plot_SCM_ozone.png){.fragment}
...
![](images/plot_SCR_ozone.png){.fragment}
:::::::::::
...
La variance des résidus $\sigma^2$ est estimée par: $$\hat \sigma^2=\frac{\sum_{i}(Y_{i}-\hat Y_{i})^2}{\mbox{nb données} - \mbox{nb paramétres estimés à partir des données}}~~~~~~~~~~\mathbb{E}(\hat\sigma^2)=\sigma^2$$
...
| Commentaires |  |  | Contrainte à poser sur les paramètres (le mieux est $\sum_{i=1}^I \alpha_i = 0$) | Contrainte à poser sur les paramètres (le mieux est $\forall i, \sum_j \alpha \beta_{ij} =0$ et $\forall j, \sum_i \alpha \beta_{ij} =0$) |
...
<!-- $y_{12}=9$ & $y_{22}=4$ & $y_{32}=1$ \\ -->
<!-- \hline -->
<!-- % Moyennes : $y_{1\bullet}=7$ & $y_{2\bullet}=3$ & $y_{3\bullet}=2$ \\ -->
<!-- \end{tabular} -->
<!-- \end{center} -->
...
<script>
  ...
  document.body.insertBefore(titleContainer, document.querySelector('.reveal'));
