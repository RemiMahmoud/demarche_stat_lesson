...
-   Aborder problèmes courants en analyse de données
...
-   **L'analyse des données**: Famille de méthodes staistiques dont les principales caractéristiques sont d'être multimensionnelles et descriptives.
...
::: r-stack
::: {layout-ncol="3"}
![](images/co2_rich.PNG){.fragment height="180"}

![](images/covid_groupe_sanguin.PNG){.fragment height="180"}

![](images/figaro.PNG){.fragment height="180"}
:::
:::
...
# # 
# ggsave(plot =plot_all, filename = "lesson/images/death_rate_all.png", dpi = "retina", width = 15, height = 8, unit = 'cm')
# ggsave(plot = plot_by_age, filename = "lesson/images/death_rate_by_age.png", dpi = "retina", width = 15, height = 8, unit = 'cm')
...
# 
# 
# ggsave(plot = plot_size_sample, filename = "lesson/images/size_sample.png", dpi = "retina", width = 15, height = 10, unit = 'cm')
...
ggsave(plot = plot_population, filename = "lesson/images/plot_population.png", dpi = "retina", width = 8, height = 4, unit = 'cm', scale = 3.5)
ggsave(plot = plot_population_and_sample, filename = "lesson/images/plot_population_and_sample.png", dpi = "retina", width = 8, height = 4, unit = 'cm', scale = 3.5)
ggsave(plot = plot_population_and_sample_inference, filename = "lesson/images/plot_population_and_sample_inference.png", dpi = "retina", width = 8, height = 4, unit = 'cm', scale = 3.5)
...
plot_summary <- data_plot_summary_stat %>% 
  ...
  geom_label(fill = "grey90",data= data_summary, aes(x = 0.7, y = quant_25), inherit.aes = FALSE, label  = latex2exp::TeX("$q_{0.25}(x)$ (premier quartile)"), size = 5.5)  +
...
ggsave(plot = plot_summary, filename = "lesson/images/plot_descriptors.png", dpi = "retina", width = 10, height = 5.5, unit = 'cm', scale = 3.5)
...
-   La visualisation dépend:
    -   nature des données (nature des variables, nb d'individus)

    -   probématique: qu'est-ce que je veux montrer ?

    -   ![](images/data-to-viz.com)
...
Sur ![](images/R_logo.svg.png){fig-align="center" width="5%"}, le package `ggplot2` permet de faire des visualisation allant du plus simple au plus compliqué.
...
## Distribution d'une variable qualitative {.scrollable}
...
::: columns
::: {.column width="50%"}
![](images/pie_bad_idea.png){width="100%" fig-align="center" height="140"}
:::

::: {.column width="50%"}
![](images/barplot_good_idea.png){.fragment width="100%" fig-align="center" height="140"}
:::
:::
:::
...
## Effet d'une / deux variable(s) quali sur une variable quanti {.scrollable}
...
![](images/plot_population_and_sample_inference.png){.fragment width="60%" height="300" fig-align="center"}

. . .

C'est toute la question des *tests statistiques*.

# Principes et application des tests statistiques
...
-   Ce que dit le TCL, c'est que peu importe la distribution de base, les moyennes empiriques d'échantillons issues cette distribution auront une distribution normale !
...
Mais dans la *vraie vie*, on aurait accès qu'à un échantillon de cette loi, par ex. de taille $n = 50$
...
# write.csv(x = data_mean_samples_all, file = "lesson/data/data_mean_samples_all.csv")
...
echantillon_student <-tibble(x= seq(-4,4, by = 0.1), y = dt(x = x,df = 111))

echantillon_student %>% 
  ggplot(aes(x = x, y=  y)) + 
  geom_line() + 
  theme_bw() + 
  stat_function(fun = dstudent_limit, geom = "area", fill = "lightblue4", alpha = 0.2) + 
  stat_function(fun = dstudent_limit_out, geom = "area", fill = "red", alpha = 0.2) + 
  geom_label(label = "Zone de non-rejet",x = 0, y=  0.2) +
  geom_label(label = "Zone de rejet",x = -2.5, y=  0.02) +
  geom_label(label = "Zone de rejet",x = 2.5, y=  0.02) +
  labs(y = "Densité", x = NULL,  y= "Loi de student à 111 ddl")
...
2.  variabilité du maximum d'ozone est faible au sein des jours pluvieux et au sein des jours secs
3.  s'il y a beaucoup de données
...
> Ici on rejet $H_0$ au seuil de 5%, les vairances sont différentes
...
-   **Erreur de première espèce**: rejeter $H_0$ alors que celle-ci est vraie: test positif
...
# Analyse de variance à un facteur {.scrollable}
...
**Variable réponse quantitative**, notée Y: maxO3 **Variable explicative qualitative à I modalités (groupes)**: direction du vent
...
On minimise le **critère des moindres carrés ordinaires**
...
{{< fa question size=1xl >}}: pourquoi carré $^2$ ?
...
::: {.column width="36%"}
```{=latex}
$$H_0 :~\mbox{"}\forall i,\ \mu_i=\mu\mbox{"}$$
$$H_1 :~\mbox{"}\exists i\ /\ \mu_i\ne \mu\mbox{"}$$
```
:::
...
meansComp(res, ~vent, adjust ="Bonerroni", graph = FALSE)
...
## Corrélation, causalité etc. {.scrollable}
...
-   A $\Rightarrow$ B et *ET* B $\Rightarrow$ A (ex. {{< fa temperature-high size=1xl >}} et CO2 )
...
![](images/spurious_cor.svg){width="60%" fig-align="center" height="250"} 
...
**Hypothèses** :$~~~H_0 : \mbox{"} \beta_1=0\mbox{"}$ contre $H_1 : \mbox{"} \beta_1 \ne 0\mbox{"}$ **Statistique de test :** $\displaystyle\frac{\hat\beta_1}{\hat\sigma_{\hat\beta_1}}$ **Loi de la statistique de test sous** $H_0$: ${\mathcal L}\left(T_{obs}=\frac{\hat\beta_1}{\hat\sigma_{\hat\beta_1}}\right)={\mathcal T}_{\nu=n-2}$
...
{{< pdf images/aber.pdf width=100% height=500 >}}
...
$\mathbb{E}(\hat{Y}_0) \sim \mathcal{N}(Y_0, \sigma \sqrt{\frac{1}{n} + \frac{(x_0 - \bar{x})^2}{\sum_{i=1}^n (x_i - \bar{x})^2)}})$
...
Prédiction d'une nouvelle valeur de Y pour un $x_0$ donné.
...
$$ \left\{
\begin{array}{l}
\forall i = 1,...,n ~~~~Y_{ij}=\beta_0+\beta_1x_{i}+\color{red}{\beta_2x_{i2}+\beta_3x_{i3}+...+\beta_px_{ip}} + \varepsilon_{i}\\
...
## Quatres types d'effet possibles {.scrollable}
...
ozone %>%
  ...
  geom_point() +
  geom_smooth(method ="lm", se= FALSE) +
  labs(title = "Effet de T9 sur le max d'ozone")
```
:::
...
#| output-location: "fragment"
#| code-fold: true
#| fig-cap: "Quali / quali sur quanti"
#| fig-align: "center"
...
#| output-location: "fragment"
#| code-fold: true
#| fig-cap: "Quanti / quali sur quanti"
#| fig-align: "center"
...
$\color{red}{\mbox{AJOUTER DECOMPO VARIABILITE}}$
...
\bf Hypothéses : $H_0$:"tous les coefficients associés aux variables de ${\mathcal V}$ sont égaux à 0" contre $H_1$ : "au moins un coefficient des variables ${\mathcal V} \neq$ 0

```{=tex}
\bf Statistique de test : $\displaystyle{F_{obs}=\frac{SC_{\mathcal V}/ddl_{\mathcal V}}{SC_R/ddl_R}=\frac{CM_{\mathcal V}}{CM_R} }$
\vfill
```
```{=tex}
\bf Loi de la statistique de test : Sous $H_0$, ${\mathcal L}(F_{obs}) = {\mathcal F}_{ddl_R}^{ddl_{\mathcal V}}$
\vfill
```
\bf Décision : $\mathbb{P}({\mathcal F}_{ddl_{\mathcal V}}^{ddl_R}>F_{obs}) < 0.05~~\Longrightarrow$ Rejet de $H_0$

. . .

::: incremental
-   Revient à choisir entre le sous-modéle sans les variables ${\mathcal V}$ ou le modéle complet

-   Si ${\mathcal V}$ contient tous les effets : revient é tester si $R^2$ est significativement différent de 0, i.e. si toutes les variables sont inutiles (versus au moins une utile)

-   On somme les degrés de liberté associés é l'ensemble ${\mathcal V}$ sachant qu'1 variable quanti é 1 ddl, 1 variable quali é $I-1$ ddl et une interaction a comme ddl le produit des ddl de chaque facteur
:::
...
{{< fa question size=1xl >}} A votre avis, quelle est la différence entre les 2 critères
...
::: {style="font-size: 70%;"}
| a                        | Constante ($\mu$ ou $\beta_0$) | Variable quanti       | Variable Quali                                                                   | Interaction quali -quali                                                                                                                  |
|---------------|---------------|---------------|---------------|---------------|
...
#lm par défaut
coef( lm(maxO3 ~ pluie, data= ozone) )


#lm en spécifiant le contraste
coef(lm(maxO3 ~ pluie, data = ozone, contrasts = list(pluie=  "contr.sum")))

# LinearModel prémâche le travaille
LinearModel(maxO3 ~ pluie, data= ozone)$Ttest
...
-   **Modèle sélectionné** $\Rightarrow$ Interprétaiton des résultats
